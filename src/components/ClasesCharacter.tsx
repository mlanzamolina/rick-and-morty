...
export default function ClasesCharacter() {
  ...
  return (
    <div>
          ...
          {character.results.map((item, index) => {
            return (
              <tr className={index % 2 ? "table-primary" : "table-info"}>
                ...
                <td>
                  <Link to={`/DetallesCharacters/${item.id}`}>{item.name}</Link>{" "}
                </td>
      ...
      <div className="btn">
        <button id="btn-1" onClick={Atras}>
          prev
        </button>
        <button id="btn-2" onClick={Siguiente}>
          next
        </button>
      </div>
    </div>
